...
CMD set -euo pipefail \
    && dotnet restore VoiceDirector.Tests/VoiceDirector.Tests.csproj \
    && dotnet test VoiceDirector.Tests/VoiceDirector.Tests.csproj --no-restore --configuration Release \
    ...
    && dotnet restore VoiceDirector/VoiceDirector.csproj -p:EnableWindowsTargeting=true \
    && dotnet build VoiceDirector/VoiceDirector.csproj --no-restore --configuration Release -p:EnableWindowsTargeting=true -o /tmp/plugin-build \
    && if [[ -d /out && -w /out ]]; then \
