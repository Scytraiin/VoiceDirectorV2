...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Update Voice Director release metadata, rebuild via Docker, and refresh "
...
def normalize_version(version: str) -> tuple[str, str, str]:
    ...
    if not re.fullmatch(r"\d+(?:\.\d+){1,3}(?:-[0-9A-Za-z.-]+)?", clean_version):
        raise ValueError("Version must look like 1.6.1, 1.7.0-beta, or 1.7.0.0.")
...
def main() -> int:
    ...
    run(["docker", "build", "-t", "voice-director-ci", "."], workspace)
